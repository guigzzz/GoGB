...
func TestRotateLeftCarryHL(t *testing.T) {
	...
	c.RotateLeftCHL()
	assert.Equal(t, c.readMemory(c.ReadHL()), byte(0))
...
func TestRotateLeftHL(t *testing.T) {
	...
	c.RotateLeftHL()
	assert.Equal(t, c.readMemory(c.ReadHL()), byte(0x22))
...
func TestRotateRightHL(t *testing.T) {
	...
	c.RotateRightHL()
	assert.Equal(t, c.readMemory(c.ReadHL()), byte(0x45))
	assertFlagsSet(t, c.reg[F])
}
